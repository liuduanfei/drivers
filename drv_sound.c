...
static struct sai_reg * sai1 = (struct sai_reg *)SAI_BASE;
...
rt_err_t SAIA_tx_dma(void)
{
    ...
    DMA2_Stream3->PAR  = (rt_uint32_t)&sai1->adr;//外设地址为:SAI1_Block_A->DR
    DMA2_Stream3->M0AR = (rt_uint32_t)_stm32_audio_play.tx_fifo;//内存1地址
    DMA2_Stream3->M1AR = (rt_uint32_t)_stm32_audio_play.tx_fifo + (TX_DMA_FIFO_SIZE / 2);//内存2地址
